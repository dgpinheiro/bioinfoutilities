...
my ($level, $input1, $input2, $beddir, $inputrna1, $inputrna2);
...
GetOptions( "h|?|help" => sub { &Usage(); },
            ...
            "i2|input2=s"=>\$input2,
            "b|beddir=s"=>\$beddir,
            "r1|inputrna1=s"=>\$inputrna1,
            "r2|inputrna2=s"=>\$inputrna2
    ) or &Usage();
...
$LOGGER->logdie("Missing inputrna1 file") unless ($inputrna1);
$LOGGER->logdie("Wrong inputrna1 file ($inputrna1)") unless (-e $inputrna1);

$LOGGER->logdie("Missing inputrna2 file") unless ($inputrna2);
$LOGGER->logdie("Wrong inputrna2 file ($inputrna2)") unless (-e $inputrna2);
...
my %datarna1;
{
    my $in1 = Bio::SeqIO->new(-file=>$inputrna1, -format=>'FASTA');
    while(my $seq = $in1->next_seq() ) {
        my $id;
        if ($seq->display_id()=~/^[^\|]+\|([^\|]+)/) {
            $id = $1;
...
my %datarna2;
{
    my $in2 = Bio::SeqIO->new(-file=>$inputrna2, -format=>'FASTA');
    while(my $seq = $in2->next_seq() ) {
        my $id;
        if ($seq->display_id()=~/^[^\|]+\|([^\|]+)/) {
            $id = $1;
    ...
    if (scalar(@r) > 1) {
        ...
        foreach my $p (@r) {
            ...
            unless (exists $rna{$p}) {
                ...
                if ($rna{$p} ne $b2) {
                    $LOGGER->logdie("Something wrong with the PEP<=>RNA relationship");
                }
            }
        ...
        if (scalar(@x) == 1) {
            ...
            if (scalar(@sel)==1) {
                ...
                $LOGGER->logwarn("Difficult to map $s1 to any of ".join(';', @r).' because '.join(';', map { $_."[".$selhash{$_}."]"  } keys %selhash).' have the same intersection length so we will check the RNA sequences');
                
                my @lasttry;
                foreach my $k (keys %selhash) {
                    if ($rna{$s1} eq $rna{$k}) {
                        push(@lasttry, $k);
                    }
                }
                ...
                elsif (scalar(@lasttry) > 1) {
                    $LOGGER->logwarn("Difficult to map $s1 to any of ".join(';', keys %selhash).' because '.join(';', @lasttry).' have the same RNA sequence');

                    print $s1,"\t",join(';', @lasttry),"\n";
                }
                else {
                    $LOGGER->logwarn("Difficult to map $s1 to any of ".join(';', keys %selhash).' because '.join(';', @lasttry)." don't have RNA sequence equal to $s1");
                ...
                $LOGGER->logwarn("Not found intersection among $s1 and any of ".join(';', @r).' so we will check the RNA sequences');
